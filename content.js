...
function cleanPage() {
  ...
  while ((node = walker.nextNode())) {
    if (node.nodeValue.match(/only|hurry|limited|left/i)) {
      node.nodeValue = node.nodeValue.replace(/only|hurry|limited|left/gi, "");
    }
  }

  document.querySelectorAll('[class*="modal"], [class*="popup"]').forEach(m => m.remove());
  ...
  widget.innerHTML += `<div style="margin-top:6px;font-size:12px;opacity:0.7;">clean mode active</div>`;
}
